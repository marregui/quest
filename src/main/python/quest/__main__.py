...
def _start_command():
    qdb_jar = _find_jar()
    if not qdb_jar:
        print("QuestDB jar not found, try updating first.")
        sys.exit(1)
    _ensure_conf_exists()
    try:
        comand = [
...
def _ensure_conf_exists() -> None:
    if not QDB_DB_CONF.exists():
        ...
        src_file = Path(quest.__file__).parent / "resources" / "conf.zip"
...
def is_qdb_jar(file_name: str) -> bool:
    return (
        ...
        and file_name.startswith("questdb-")
        and "-tests" not in file_name
    )
